...
func TestCompileWithReplacedGoSubpackage(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "demo-factory")
	require.NoError(t, os.MkdirAll(dir, 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "factory.ub"),
		factorySource("imports: { fs: 'example.com/repo//go/fs' }\n"), 0o644))

	moduleDir := t.TempDir()
	require.NoError(t, os.MkdirAll(filepath.Join(moduleDir, "fs"), 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(moduleDir, "go.mod"),
		[]byte("module example.com/repo/go\n\ngo 1.26\n"), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(moduleDir, "fs", "library.go"),
		validGoLibrarySource("fs"), 0o644))

	require.NoError(t, os.WriteFile(filepath.Join(dir, deps.ManifestFileName),
		manifestSource("requires: {}\nreplace: { 'example.com/repo//go': '"+
			moduleDir+"' }\n"), 0o644))
...
`
	require.Equal(t, wantMain, string(mainGo))
