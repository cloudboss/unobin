...
func TestVersions(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "subdir ignores root tags when no project tags exist",
			dep:  net,
			tags: []string{"v1.0.0", "v1.1.0", "db/v3.0.0"},
			want: nil,
		},
		{
			name: "subdir ignores parent project tags",
...
func TestResolveVersion(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "subdir dep does not use root tags", dep: sub, query: "latest",
			tags: []string{"v1.0.0", "v2.0.0"}, wantErr: true},
