...
// A factory-level check keeps the input root, the name an operator sets;
...
func (d FieldDisplay) name(f string) string {
	if d == DisplayNodeRelative {
		return strings.TrimPrefix(f, "input.")
	}
	return f
}
...
func ConstraintFieldRoots(c ConstraintEntry) []string {
	...
	for _, e := range exprs {
		Walk(e, func(x Expr) {
			dp, ok := x.(*DotPath)
			if !ok || dp.Root == nil || dp.Root.Name != "input" || len(dp.Segments) == 0 {
				return
			}
...
// constraintFieldName renders a `fields:` element to the input reference
// it must be: input.vpc-id names an input and input.code.inline a field
...
// (input.listeners[0].cert) or splat every element (input.replicas[*].host);
...
func constraintFieldName(e Expr) (string, bool) {
	v, ok := e.(*DotPath)
	if !ok || v.Root == nil || v.Root.Name != "input" || len(v.Segments) == 0 {
		return "", false
	}
	// The first segment must name an input; an index or splat applies
	// to a field, never to input itself.
...
// lookupPath reads a field value by its input reference, stepping into
// nested maps for a name like "input.code.inline" and into list elements
// for an indexed segment like "input.listeners[0].cert". The leading input
// segment resolves to the values themselves; it is part of the name so
// a field reads the same everywhere, never input-stripped in one place
// and rooted in another. found is false for a name not rooted at input,
...
// path from the input down: input.rules[1].transitions[0].
