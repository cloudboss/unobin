...
func Generate(in Input) ([]byte, error) {
	...
	data := struct {
		...
		ConstraintAliases: sortedConstraintAliases(in.GoConstraints),
...
// sortedConstraintAliases returns the Go-library aliases that carry at
// least one constrained type, sorted for stable output.
func sortedConstraintAliases(m map[string]map[string][]lang.ConstraintSpec) []string {
	keys := make([]string, 0, len(m))
	for k, v := range m {
		if len(v) > 0 {
			keys = append(keys, k)
...
func sortedSpecKeys(m map[string][]lang.ConstraintSpec) []string {
	keys := make([]string, 0, len(m))
	for k := range m {
		keys = append(keys, k)
	}
...
import (
{{if .ConstraintAliases}}	"github.com/cloudboss/unobin/pkg/lang"
