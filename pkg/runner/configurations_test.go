...
`)
	out, err := loadConfigurations(path, map[string]*runtime.Module{
...
func TestLoadConfigurationsErrorsWhenModuleRequiresOneAndConfigIsAbsent(t *testing.T) {
	_, err := loadConfigurations("", map[string]*runtime.Module{
		"aws": awsModuleWithConfig(),
	})
	require.Error(t, err)
...
`)
	_, err := loadConfigurations(path, map[string]*runtime.Module{
		"aws": awsModuleWithConfig(),
	})
	require.Error(t, err)
	require.Contains(t, err.Error(), "configurations.aws")
...
func TestLoadConfigurationsReturnsNilWhenNoModuleNeedsOne(t *testing.T) {
	out, err := loadConfigurations("", map[string]*runtime.Module{
