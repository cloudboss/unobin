...
// sub-package.
func ResourceFile(rs ResourceSchema, source string) ([]byte, error) {
	var b bytes.Buffer

	writeGeneratedComment(&b, source)
...
// sub-package.
func DataSourceFile(ds DataSourceSchema, source string) ([]byte, error) {
	var b bytes.Buffer
...
func writeGeneratedComment(b *bytes.Buffer, source string) {
	if source == "" {
		source = "unknown"
	}
